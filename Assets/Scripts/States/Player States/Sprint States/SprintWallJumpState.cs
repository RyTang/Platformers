...
public class SprintWallJumpState : BaseState<PlayerController>
{
    ...
    public override void CheckStateTransition()
    {
        ...
        if (Runner.GetWallCheck().Check()){
            CurrentSuperState.SetSubState(CurrentSuperState.GetState(typeof(SprintWallClingState))); // FIXME: Seems to be an Issue where If wall jumping without pressing direction, it won't trigger the wall cling state
        }
    }
    ...
    public override void OnStateCollisionEnter(Collision2D collision)
    {
        if (Runner.GetGroundCheck().Check() && !(Runner.GetWallCheck().Check() && rb2d.velocity.x != 0)) { // Problem Statement: Sometimes it will trigger the Land State when falling and colliding with the fall whereas it should have been a wall Cling State
            CurrentSuperState.SetSubState(CurrentSuperState.GetState(typeof(SprintLandState)), collision.relativeVelocity.y);
        }
    }
}
