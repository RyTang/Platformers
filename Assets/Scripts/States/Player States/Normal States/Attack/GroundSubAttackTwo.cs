...
public class GroundSubAttackTwo : BaseState<PlayerController>, IAttack
{
    ...
    public override void CheckStateTransition()
    {
        if (attackControl > 0 && Runner.GetAnimator().GetBool(PlayerAnimation.canAttackBool) && Runner.GetAnimator().GetBool(PlayerAnimation.isAttackingBool)){
            CurrentSuperState.SetSubState(CurrentSuperState.GetState(typeof(GroundSubAttackThree)));
        }
