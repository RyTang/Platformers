...
public class PlayerController : BaseCharacter, IBasicMovement, IWallMovement
{   
    enum PlayerState {
        Idle,
        Normal,
    ...
    public void TouchingWall(bool touching)
    {
        if (touching && !touchingGround){
            ...
            ChangePlayerState(PlayerState.WallSliding);
            clingingWall = true;
            canWallJump = true;
            isJumping = false;
        }
    ...
    {
        if (clingingWall && !isWallJumping && verticalControl > 0){
            ...
            Invoke(nameof(StopWallJumping), wallJumpDuration * Time.deltaTime);
        }
